  ...
  class Storage {
    constructor(options = {}) {
      ...
      this._mapPromise = this.getItem('map').then(map => {
        this._m = this._checkMap(map && JSON.parse(map) || {}); // delete this._mapPromise;
      });
    }
    ...
    _lookupGlobalItem(params) {
      ...
      return this.getItem(key).then(ret => this._loadGlobalItem(_objectSpread({
        ret
      }, params)));
    }
    ...
    _lookUpInMap(params) {
      ...
      if (m[newId] !== undefined) {
        return this.getItem('map_' + m[newId]).then(ret => this._loadMapItem(_objectSpread({
          ret
        }, params)));
      }
